...
coord = [flipud(coord(2:end,:)) coord*[1 00 -1]]
...
torsionLength = sum(math.sqrt(sum(diff(box)**2,2)))
...
for i=1:4
    l = math.sqrt(sum(diff(seg{i})**2,2)) # Segment lengths
...
# Skin
box = coord*chord
skinLength = sum(math.sqrt(sum(diff(box)**2,2)))
