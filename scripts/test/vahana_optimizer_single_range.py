...
if __name__ == '__main__':
    ...
    top = Problem()
    top.root = TopLevelSystem()
    
    # SubProblem: set up the optimizer
    top.driver = ScipyOptimizer()
    top.driver.options['optimizer'] = 'COBYLA'  # The 'COBYLA' optimizer is supported by OpenMETA. 
                                                # Unlike the 'SLSQP' optimizer, the 'COBYLA' optimizer doesn't require a Jacobian matrix.
    top.driver.options['disp'] = True  # enable optimizer output
    top.driver.options['maxiter'] = 100
    top.driver.options['tol'] = 1000.0
    top.driver.opt_settings['rhobeg'] = 100.0
    
    # SubProblem: set design variables for sub.driver
    top.driver.add_desvar('indep2.rProp', lower=0.3, upper=2.0)
    top.driver.add_desvar('indep3.cruiseSpeed', lower=39.0, upper=80.0)
    top.driver.add_desvar('indep4.batteryMass', lower=10.0, upper=999.0)
    top.driver.add_desvar('indep5.motorMass', lower=1.0, upper=999.0)
    top.driver.add_desvar('indep6.mtom', lower=100.0, upper=9999.0)
    
    # SubProblem: set design objectives
    top.driver.add_objective('OperatingCost.C_costPerFlight')
    ...
    top.driver.add_constraint('indep2.rProp', lower=0.3, upper=2.0)
    top.driver.add_constraint('indep3.cruiseSpeed', lower=39.0, upper=80.0)
    top.driver.add_constraint('indep4.batteryMass', lower=10.0, upper=999.0)
    top.driver.add_constraint('indep5.motorMass', lower=1.0, upper=999.0)
    top.driver.add_constraint('indep6.mtom', lower=100.0, upper=9999.0)
    
    # SubProblem: set design constraints
    top.driver.add_constraint('con1.c', lower=0.0)
    top.driver.add_constraint('con2.c', lower=0.0)
    top.driver.add_constraint('con3.c', lower=0.0)
   
    # Data collection
    recorder = SqliteRecorder('top')
    ...
    # db = sqlitedict.SqliteDict( 'top', 'iterations' )
